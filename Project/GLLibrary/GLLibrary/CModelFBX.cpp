...
CModelFBX::AnimationInfo::AnimationInfo(FbxManager* m, FbxScene* s, FbxAnimStack* a, FbxTakeInfo* t)
{
	...
	double r= m_lScene->GetGlobalSettings().GetCustomFrameRate();
...
void CModelFBX::LoadTexture(CTexture** tex, char* name,FbxProperty& property) {
	...
	for (int i = 0; i < numGeneralTexture; ++i) {
		...
		*tex = new CTexture();
		...
		strcpy_s(name, NAME_STR_SIZE, (char*)lFileTexture->GetRelativeFileName());
		char* sc = strchr(name, ':');
		if (sc) {
			char* p = strrchr(name, '/');
			if (!p) p = strrchr(name, '\\');
			strcpy_s(name, NAME_STR_SIZE, p);
		}
		char str[PATH_SIZE];
		...
		if (!(*tex)->Load(str)) {
			if (!(*tex)->Load((char*)lFileTexture->GetFileName())) {
...
int CModelFBX::AddMaterial(FbxSurfaceMaterial *pMat) {
	...
	if (pMat->GetClassId().Is(FbxSurfacePhong::ClassId))
	{
		...
		float f = m->SpecularFactor.Get();
		mat->m_specular.r = static_cast<float>(m->Specular.Get()[0])*f;
		mat->m_specular.g = static_cast<float>(m->Specular.Get()[1])*f;
		mat->m_specular.b = static_cast<float>(m->Specular.Get()[2])*f;


	//	auto prop = pMat->FindProperty(FbxSurfaceMaterial::sSpecular);
	//	mat->m_specular = CVector3D(prop.Get<FbxDouble3>()[0], prop.Get<FbxDouble3>()[1], prop.Get<FbxDouble3>()[2]);
	...
	LoadTexture(&mat->mp_texture, mat->m_texture_name, pMat->FindProperty(FbxSurfaceMaterial::sDiffuse));
	LoadTexture(&mat->mp_normal_map, mat->m_normal_map_name, pMat->FindProperty(FbxSurfaceMaterial::sNormalMap));
...
void CMeshFBX::Render(const CMatrix& mv, const CMatrix& m, std::vector<CMaterial*>& materialList,CMatrix* boneMatrix,float blend) {
	...
	for (int i = 0; i < m_material_cnt; i++) {
		...
		CShader* s = CShader::GetInstance(mat->m_shader_name);
		s->Enable();
