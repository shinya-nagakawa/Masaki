...
PhysicsTower::PhysicsTower(Kinds kinds) : TowerBase()
	...
	m_kinds = kinds;
	//State登録
	m_stateList[(int)TowerState::eState_BuildBefore] = new Physics_BuildBefore(this);
	m_stateList[(int)TowerState::eState_BuildAfter]  = new Physics_BuildAfter(this);
	m_stateList[(int)TowerState::eState_Broken]      = new Physics_Broken(this);
}
...
void PhysicsTower::CheckTarget(){
	//現在の狙っている敵のIDで検索
	Task* t = TaskManager::FindObjectbyID(m_target_ID);
	//EnemyBase型にキャスト
	EnemyBase* e = static_cast<EnemyBase*>(t);
	//ターゲットが存在していたら
	if (e) {
...
bool PhysicsTower::IsTargetRange(CVector3D vec){
